...
def create_model(y):
    ...
    #stroke_read_model.add(Dropout(0.14))
    stroke_read_model.add(Conv2D(96, (3,3)))
    stroke_read_model.add(MaxPooling2D(3,3))
    
    stroke_read_model.add(Conv2D(128, (3,3)))
    stroke_read_model.add(MaxPooling2D(3,3))
    stroke_read_model.add(Flatten())
    
#    stroke_read_model.add(LSTM(128, return_sequences = True))
    #stroke_read_model.add(Dropout(0.21))
#    stroke_read_model.add(LSTM(128, return_sequences = False))
    ...
    stroke_read_model.add(Dense(512))
    #stroke_read_model.add(Dropout(0.16))
...
if __name__ == '__main__':
    data = pd.read_csv(r"all_10%.csv", index_col=0)
    ...
    history = model.fit(X_train, np.array(y_train), epochs=100,
                        validation_data=(X_test, np.array(y_test)), batch_size=128, verbose=2)
